...
def proxl1s(p,mu=1.0,dims=None,vdims=(),copy=True,tau=1.0):
    ...
    if mu == 0:
        z = torch.zeros(z.shape, device = p.get_device())
    else:
        if vdims:
            z/= torch.clamp( torch.sqrt(torch.sum(torch.square(p),dim=vdims,keepdims=True) )/mu , min=1.0)
        else:
            z /= torch.clamp( torch.abs(p)/mu , min=1.0)
...
def l2nsq(x,mshift=0.0,dims = None):
    return torch.sum(torch.square(torch.abs(x-mshift)), dim = dims,keepdims=True)

def l1nrm(x,mshift=0.0,dims=None,vdims=(),eps=0):

    if vdims or eps>0:
        return torch.sum(torch.sqrt(torch.sum(torch.square(x-mshift),dim=vdims)), dim=dims,keepdims=True)
    else:
        return torch.sum(torch.abs(x-mshift),dim=dims,keepdims=True)
...
class nfun(object):
    ...
    def __init__(self,ntype,npar=1.0,mshift=0.0,dims=None,vdims=(), mask=False,eps=0.1,delta=1.0,l1eps_par=0.1):
        ...
        self.mshift = mshift.clone().detach() #Minus-shift: We consider N(x-mshift)
        ...
        #Set evaluation
        if ntype == 'l1':
            def val(x): return self.npar*l1nrm(x,mshift=self.mshift, dims = dims, vdims=self.vdims)
            def prox(x,ppar): return shrink(x,tau=self.npar*ppar,mshift=self.mshift, vdims=self.vdims)
            def subgrad(x):
                if vdims:
                    pointwise_2norm = torch.sqrt(torch.sum(torch.power(x-self.mshift,2), dim=vdims, keepdims=True))
                    divider = pointwise_2norm + 1.0*(pointwise_2norm<1e-10)
                    subgrad = self.npar*(x-self.mshift)/divider
                else:
                    subgrad = self.npar*torch.sign(x-self.mshift)
                return subgrad

        if ntype == 'l2sq':
            def val(x): return 0.5*self.npar*l2nsq(x,mshift=self.mshift, dims = dims) 
...
def test_adj(fwd,adj,dim1,*dim2):
        ...
        x = torch.tensor(np.random.rand(*dim1))
        y = torch.tensor(np.random.rand(*dim2))

        s1 = (fwd(x)*y).sum()
        s2 = (x*adj(y)).sum()
        
        print('Abs err: ' + str(torch.abs(s1-s2)))
        print('Rel err: ' + str( torch.abs(s1-s2)/torch.abs(x).sum() ))
...
class gradient_1d(object):
    ...
    def fwd(self,x):
        ...
        z = torch.zeros(self.outdim, device = x.get_device())
    ...
    def adj(self,p):
        ...
        x = torch.zeros(self.indim, device = p.get_device())
